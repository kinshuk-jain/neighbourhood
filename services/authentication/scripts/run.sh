...
function display_usage {
  echo "Usage: $0 <invoke|run> <verify-access-level|generate-login-token|verify-login-token>"
  exit 1
}
...
if ! [[ "$FUNCTION" =~ ^(verify-access-level|generate-login-token|verify-login-token)$ && "$COMMAND" =~ ^(invoke|run)$ ]]; then
  display_usage
fi
