...
// TODO: signout function, signup function
...
import { getAuthCodeData } from './db'
...
const myHandler: APIGatewayProxyHandler = async (
  ...
  try {
    ...
    if (queryParams.grant_type.toLowerCase() === 'refresh_token') {
      ...
      // do code_challenge_method operation on code_verifier and check if it matches code challnge
      // invalidate auth code
      // issue access_token, refresh_token
      // reply with access_token, refresh_token, expires_in
    } else {
