...
function display_usage {
  echo "Usage: $0 <invoke|run> <create|list|delete|get|update|create-reply|update-reply>"
  exit 1
}
...
if ! [[ "$FUNCTION" =~ ^(create|list|delete|get|update|create-reply|update-reply)$ && "$COMMAND" =~ ^(invoke|run)$ ]]; then
  display_usage
fi
