...
function display_usage {
  echo "Usage: $0 <deploy> <create|list|delete|get|update|create-reply|update-reply|delete-reply|get-reply|update-name>"
  exit 1
}
...
if ! [[ "$FUNCTION" =~ ^(create|list|delete|get|update|create-reply|update-reply|delete-reply|get-reply|update-name)$ && "$COMMAND" =~ ^(deploy)$ ]]; then
  display_usage
fi
