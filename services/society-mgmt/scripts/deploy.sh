...
function display_usage {
  echo "Usage: $0 <deploy> <create-society|delete-society|read-society|update-society>"
  exit 1
}
...
if ! [[ "$FUNCTION" =~ ^(create-society|delete-society|read-society|update-society)$ && "$COMMAND" =~ ^(deploy)$ ]]; then
  display_usage
fi
